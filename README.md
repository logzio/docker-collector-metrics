...
| DEBUG (Default: `false`) | Set to `true` if you want metricbeat to run in debug mode.<br/> **Note:** This option will generate quite a lot of debugging output, so you should use it if an error occurs while running the docker-collector in production.  |
